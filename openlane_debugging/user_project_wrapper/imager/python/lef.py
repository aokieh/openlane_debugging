...
with open(filename, "w") as f:
    ...
    # Bottom Horizontal 
    f.write(f"\t\t\t\tRECT 0.000 17.770 {macro_width:.3f} 20.770 ;\n")
    # Top Horizontal
    f.write(f"\t\t\t\tRECT 0.000 {macro_height - 20.770:.3f} {macro_width:.3f} {macro_height - 17.770:.3f} ;\n")
    f.write(f"\t\t\tLAYER met4 ;\n")
    # Left Vertical
    f.write(f"\t\t\t\tRECT 11.525 0.000 12.945 {macro_height:.3f} ;\n")
    # Right Vertical
    f.write(f"\t\t\t\tRECT {macro_width - 12.945:.3f} 0.000 {macro_width - 11.525:.3f} {macro_height:.3f} ;\n")
    f.write("\t\tEND\n\tEND vdda1\n")

    f.write("\n\tPIN vssa1\n\t\tDIRECTION INOUT ;\n\t\tUSE GROUND ;\n\t\tPORT\n")
    f.write(f"\t\t\tLAYER met5 ;\n")
    # Bottom Horizontal
    f.write(f"\t\t\t\tRECT 0.000 22.470 {macro_width:.3f} 25.470 ;\n")
    # Top Horizontal
    f.write(f"\t\t\t\tRECT 0.000 {macro_height - 25.470:.3f} {macro_width:.3f} {macro_height - 22.470:.3f} ;\n")
    f.write(f"\t\t\tLAYER met4 ;\n")
    # Left Vertical
    f.write(f"\t\t\t\tRECT 5.400 0.000 6.820 {macro_height:.3f} ;\n")
    # Right Vertical
    f.write(f"\t\t\t\tRECT {macro_width - 6.820:.3f} 0.000 {macro_width - 5.400:.3f} {macro_height:.3f} ;\n")
    f.write("\t\tEND\n\tEND vssa1\n")
